          ...
          <ul className="mt-8 grid gap-5 sm:grid-cols-2">
            {WHAT_IT_DOES.map((item) => (
              <Card as="li" key={item.title} className="bg-canvas">
                <span className="inline-flex h-9 w-9 items-center justify-center rounded-[var(--radius-control)] border border-line bg-surface text-brand-ink">
                  <Icon name={item.icon} size="1.25rem" />
                </span>
                <h3 className="mt-4 text-lg font-semibold tracking-tight">{item.title}</h3>
                <p className="dl-measure mt-2 text-ink-muted">{item.body}</p>
              </Card>
            ))}
          </ul>
        </div>
        ...
        <div className="mx-auto grid max-w-5xl gap-8 px-5 py-14 sm:grid-cols-2">
          <div>
            ...
            <p className="mt-4 text-ink-muted">
              ...
              table. Status is never shown by colour alone. There is always a label and an icon too.
            </p>
            <p className="mt-4">
