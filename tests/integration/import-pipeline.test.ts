...
import { commitImport, ImportError, prepareImport, undoImport } from '@/lib/services/import-service';
import { prisma, createTestUser, deleteTestUser, toArrayBuffer, type SeededUser } from './test-helpers';
...
describe('end-to-end import: ml/data sample files', () => {
  it('imports ml/data/sample_logs.csv with correct sourcing, batch counts and provenance', async () => {
    ...
    const prepared = await prepareImport({ userId: user.id, filename: 'sample_logs.csv', mimeType: 'text/csv', bytes, timezone: 'UTC' });
    ...
    const rows = await prisma.glucoseReading.findMany({ where: { userId: user.id, importBatchId: batchId } });
    ...
    const meds = await prisma.medicationEvent.findMany({ where: { userId: user.id, source: 'IMPORT' } });
  ...
  it('imports ml/data/sample_glucose_data.csv (wide format) with correct sourcing', async () => {
    ...
    const prepared = await prepareImport({ userId: user.id, filename: 'sample_glucose_data.csv', mimeType: 'text/csv', bytes, timezone: 'UTC' });
    expect(prepared.connectorId).toBe('dialog-legacy');
    ...
    const glucoseRows = await prisma.glucoseReading.count({ where: { userId: user.id, importBatchId: batchId, source: 'IMPORT' } });
...
describe('re-importing the same file', () => {
  it('adds zero rows the second time and reports duplicates; total row count is unchanged', async () => {
    ...
    const first = await prepareImport({ userId: user.id, filename: 'sample_logs.csv', mimeType: 'text/csv', bytes, timezone: 'UTC' });
    const firstCommit = await commitImport({ userId: user.id, filename: 'sample_logs.csv', mimeType: 'text/csv', byteSize: bytes.byteLength, prepared: first });
    ...
    const second = await prepareImport({ userId: user.id, filename: 'sample_logs.csv', mimeType: 'text/csv', bytes, timezone: 'UTC' });
    expect(second.summary.rowsImported).toBe(0);
    expect(second.summary.rowsDuplicate).toBe(first.summary.rowsImported);

    const secondCommit = await commitImport({ userId: user.id, filename: 'sample_logs.csv', mimeType: 'text/csv', byteSize: bytes.byteLength, prepared: second });
...
describe('malformed and all-invalid files', () => {
  it('fails gracefully (ImportError) for a corrupt/unrecognisable file, without writing anything', async () => {
    ...
    await expect(
      prepareImport({ userId: user.id, filename: 'broken.json', mimeType: 'application/json', bytes, timezone: 'UTC' }),
  ...
  it('rejects a file whose layout no connector recognises, without writing anything', async () => {
    ...
    const garbage = Buffer.from('this is just some prose, not tabular health data at all\nmore prose here\n', 'utf8');
    const bytes = toArrayBuffer(garbage);
    await expect(
      prepareImport({ userId: user.id, filename: 'notes.csv', mimeType: 'text/csv', bytes, timezone: 'UTC' }),
    ).rejects.toBeInstanceOf(ImportError);
    expect(await prisma.importBatch.count({ where: { userId: user.id } })).toBe(0);
  });
  ...
  it('a file where every row is invalid parses with zero fresh records and every row recorded as an issue — never a partial silent write', async () => {
    ...
    const prepared = await prepareImport({ userId: user.id, filename: 'all-invalid.csv', mimeType: 'text/csv', bytes, timezone: 'UTC' });
...
describe('mmol/L file is converted to mg/dL on storage', () => {
  it('stores canonical mg/dL values converted from the mmol/L source file', async () => {
    ...
    const prepared = await prepareImport({ userId: user.id, filename: 'mmol-readings.csv', mimeType: 'text/csv', bytes, timezone: 'UTC' });
    expect(prepared.summary.detectedUnit).toBe('MMOLL');
    expect(prepared.fresh).toHaveLength(3);

    await commitImport({ userId: user.id, filename: 'mmol-readings.csv', mimeType: 'text/csv', byteSize: bytes.byteLength, prepared });

    const rows = await prisma.glucoseReading.findMany({ where: { userId: user.id }, orderBy: { takenAt: 'asc' } });
...
describe('undoImport', () => {
  it("removes exactly the batch's rows and nothing else, including nothing belonging to another user", async () => {
    ...
    await prisma.glucoseReading.create({
      data: { userId: other.id, takenAt: new Date(), valueMgdl: 222, dedupeKey: 'undo-other-manual' },
    });

    const bytes = readBytes('ml/data/sample_logs.csv');
    const prepared = await prepareImport({ userId: owner.id, filename: 'sample_logs.csv', mimeType: 'text/csv', bytes, timezone: 'UTC' });
    const { batchId, imported } = await commitImport({ userId: owner.id, filename: 'sample_logs.csv', mimeType: 'text/csv', byteSize: bytes.byteLength, prepared });
    expect(imported).toBeGreaterThan(0);
    ...
    expect(await prisma.glucoseReading.count({ where: { userId: owner.id, importBatchId: batchId } })).toBe(0);

    // The pre-existing manual row for the owner survives.
    expect(await prisma.glucoseReading.count({ where: { userId: owner.id, valueMgdl: 111 } })).toBe(1);
    // The other user's row is completely untouched.
    expect(await prisma.glucoseReading.count({ where: { userId: other.id, valueMgdl: 222 } })).toBe(1);
  });
  ...
  it('undoImport for a batch owned by someone else does nothing and returns 0', async () => {
    ...
    expect(await prisma.glucoseReading.count({ where: { userId: owner.id, importBatchId: batchId } })).toBe(imported);
  });
});
