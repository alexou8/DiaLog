...
import { deleteOwnedRecord, loadAnalyticsWindow, pageGlucose, recordCounts } from '@/lib/db/health-records';
...
beforeAll(async () => {
  ...
  for (const [seed, user] of [
    ...
    await prisma.meal.create({
      data: { userId: user.user.id, takenAt, description: `meal-${seed}`, mealType: 'LUNCH', carbsG: seed, dedupeKey: `authz-meal-${seed}` },
    });
    await prisma.exerciseSession.create({
      data: { userId: user.user.id, takenAt, activity: `activity-${seed}`, durationMin: seed, dedupeKey: `authz-ex-${seed}` },
    });
    await prisma.sleepSession.create({
      data: { userId: user.user.id, takenAt, endedAt: new Date(takenAt.getTime() + seed * 60_000), durationMin: seed, dedupeKey: `authz-sleep-${seed}` },
    });
    ...
    await prisma.bloodPressureMeasurement.create({
      data: { userId: user.user.id, takenAt, systolic: 100 + (seed % 100), diastolic: 70, dedupeKey: `authz-bp-${seed}` },
    });
    await prisma.moodEntry.create({
      data: { userId: user.user.id, takenAt, mood: 3, note: `mood-note-${seed}`, dedupeKey: `authz-mood-${seed}` },
    });
    await prisma.noteEntry.create({
      data: { userId: user.user.id, takenAt, text: `note-${seed}`, dedupeKey: `authz-note-${seed}` },
    });
  }
});
...
describe('deleteOwnedRecord cross-user isolation', () => {
  it('user A cannot delete user B rows of any record type; the rows survive', async () => {
    const bGlucose = await prisma.glucoseReading.findFirstOrThrow({ where: { userId: userB.user.id, valueMgdl: 999 } });
    const bMeal = await prisma.meal.findFirstOrThrow({ where: { userId: userB.user.id, description: 'meal-999' } });
    const bExercise = await prisma.exerciseSession.findFirstOrThrow({ where: { userId: userB.user.id, activity: 'activity-999' } });
    const bSleep = await prisma.sleepSession.findFirstOrThrow({ where: { userId: userB.user.id, durationMin: 999 } });
    const bMedication = await prisma.medicationEvent.findFirstOrThrow({ where: { userId: userB.user.id, name: 'med-999' } });
    const bWeight = await prisma.weightMeasurement.findFirstOrThrow({ where: { userId: userB.user.id, weightKg: 999 } });
    const bBp = await prisma.bloodPressureMeasurement.findFirstOrThrow({ where: { userId: userB.user.id } });
    const bMood = await prisma.moodEntry.findFirstOrThrow({ where: { userId: userB.user.id, note: 'mood-note-999' } });
    ...
    expect(await prisma.medicationEvent.findUnique({ where: { id: bMedication.id } })).not.toBeNull();
    expect(await prisma.weightMeasurement.findUnique({ where: { id: bWeight.id } })).not.toBeNull();
    expect(await prisma.bloodPressureMeasurement.findUnique({ where: { id: bBp.id } })).not.toBeNull();
  ...
  it('user B cannot delete user A rows either (symmetry)', async () => {
    const aGlucose = await prisma.glucoseReading.findFirstOrThrow({ where: { userId: userA.user.id, valueMgdl: 100 } });
...
describe('pageGlucose cross-user isolation', () => {
  it("never returns B's rows in A's page, even filtered by B's known ids", async () => {
    ...
    const bIds = new Set((await prisma.glucoseReading.findMany({ where: { userId: userB.user.id } })).map((r) => r.id));
...
describe('recordCounts cross-user isolation', () => {
  it("A's counts never include B's rows", async () => {
    ...
    await prisma.glucoseReading.create({
      data: { userId: userB.user.id, takenAt, valueMgdl: 999, dedupeKey: 'authz-glucose-999-restored' },
    });
  });
});
...
describe('import dedupe-key isolation', () => {
  it("A importing a file cannot learn B has the same reading (A's existing-key lookup never includes B's keys)", async () => {
    ...
    const bKeys = new Set(
      (await prisma.glucoseReading.findMany({ where: { userId: userB.user.id }, select: { dedupeKey: true } })).map(
        (r) => r.dedupeKey,
      ),
    );
    const bKey = [...bKeys][0];
    ...
    const aExistingKeys = new Set(
      (await prisma.glucoseReading.findMany({ where: { userId: userA.user.id }, select: { dedupeKey: true } })).map(
        (r) => r.dedupeKey,
      ),
    );
...
describe('export isolation', () => {
  it("buildJsonExport for A contains none of B's data", async () => {
    ...
    expect(exportA.records.glucose.some((r) => (r as { valueMgdl: number }).valueMgdl === 999)).toBe(false);
    expect(exportA.records.meals.some((m) => (m as { description: string }).description === 'meal-999')).toBe(false);
    expect(exportA.records.exercise.some((e) => (e as { activity: string }).activity === 'activity-999')).toBe(false);
    expect(exportA.records.medications.some((m) => (m as { name: string }).name === 'med-999')).toBe(false);
    expect(exportA.records.weight.some((w) => (w as { weightKg: number }).weightKg === 999)).toBe(false);
    expect(exportA.records.moods.some((m) => (m as { note: string | null }).note === 'mood-note-999')).toBe(false);
    expect(exportA.records.notes.some((n) => (n as { text: string }).text === 'note-999')).toBe(false);
    ...
    const bRowIds = new Set(
      (await prisma.glucoseReading.findMany({ where: { userId: userB.user.id }, select: { id: true } })).map((r) => r.id),
    );
