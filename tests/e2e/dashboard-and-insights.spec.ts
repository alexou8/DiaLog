...
test.describe('dashboard and insights (demo account)', () => {
  ...
  test('dashboard shows the latest reading, 30-day stats, and a timeline chart with a table disclosure', async ({
    ...
    const table = page.locator('table').filter({ has: page.getByRole('columnheader', { name: 'When' }) });
  ...
  test('insights page shows observations with evidence badges and an expandable "why" panel', async ({ page }) => {
    await page.goto('/app/insights');
    await expect(page.getByRole('heading', { name: 'Insights' })).toBeVisible();

    const observationCards = page.locator('li').filter({ has: page.getByText('Why am I seeing this?') });
    ...
    await expect(observationCards.first().getByText(/Records compared|record/i).first()).toBeVisible();
  ...
  test('reports page renders the weekly report and clinician questions', async ({ page }) => {
    ...
    await expect(
      page.getByRole('heading', { name: 'Questions you might raise with your healthcare professional' }),
    ).toBeVisible();
  });
});
