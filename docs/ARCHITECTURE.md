...
- **Edge middleware** (`middleware.ts`) is a fast-rejection guard only — it redirects unauthenticated visitors away from `/app/*`, and does **nothing else**. It deliberately does not bounce an apparently-signed-in visitor away from `/sign-in`/`/sign-up`: the edge cannot read `User.tokenVersion`, so a cookie revoked by a password change or "sign out everywhere" still parses as valid there. Redirecting on that basis trapped remotely-signed-out devices in an `ERR_TOO_MANY_REDIRECTS` loop, locking them out of the one page that could fix it. That convenience redirect lives in the auth pages themselves, where `getCurrentUser()` can actually verify revocation. Every server component and action independently re-verifies the session and scopes its own queries; `requireUser()` is the real authorization boundary.
