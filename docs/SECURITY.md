...
## Known issue: settings actions whose response the client drops

Two related failures were measured while building federated sign-in. In both, the mutation lands in the database and the server answers correctly — only the browser never applies the response, leaving a disabled button and no confirmation.

**Bumping `tokenVersion` in a server action.** It invalidates the cookie the current request arrived with, and Next re-renders the page after the action returns. Measured over ten consecutive attempts on one account: `changePasswordAction`, which bumps, failed to confirm 6 times out of 10; the same form and action setting a _first_ password, which does not bump, failed 0 out of 10. This predates federated sign-in and is not fixed here — signing other devices out is the point of a password change, so the bump stays. Setting a first password does not bump, which is correct on its own terms: nothing was compromised, and a Google-only account has no password sessions to sign out.

**Redirecting to the route the action was submitted from.** Disconnecting Google redirected back to `/app/settings` with a notice in the query string, and the client router dropped the redirect roughly 3 times in 10 — while the identical code redirecting to any other route passed 40 attempts running. Disconnecting is therefore a route handler (`app/api/auth/google/disconnect/route.ts`) posted to by a plain form, so the browser follows the 303 itself and no client-side transition is involved. Connecting already worked this way, so the two halves of the flow now match. Route handlers do not get Next's automatic CSRF protection, so that handler checks the `Origin` header itself.
